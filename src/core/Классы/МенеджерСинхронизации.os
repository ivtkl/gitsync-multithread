...
Перем КоличествоПотоковВыполнения; // Число, количество параллельно выполняемых потоков
...
// Устанавливает количетство потоков выполнения
//
// Параметры:
//   НовыйКоличествоПотоков - Число - количество параллельно выполняемых потоков
//
// Возвращаемое значение:
//   Объект.МенеджерСинхронизации - ссылка на текущий объект класса <МенеджерСинхронизации>
//
Функция КоличествоПотоков(Знач НовыйКоличествоПотоков) Экспорт
	
	КоличествоПотоковВыполнения = НовыйКоличествоПотоков;
	Возврат ЭтотОбъект;
...
Процедура Синхронизировать(Знач КаталогРабочейКопии, Знач ПутьКХранилищу,
	...
	Если КоличествоПотоковВыполнения <> 1 Тогда
		
		Лог.Информация("Запущено выполнение в многопоточном режиме");
		Лог.Информация("->	Количество потоков = " + КоличествоПотоковВыполнения);
		КоличествоВерсий = МаксимальнаяВерсияДляРазбора - СледующаяВерсия + 1;
		КоличествоИтераций = Цел(КоличествоВерсий / КоличествоПотоковВыполнения);
		Если КоличествоИтераций <> КоличествоВерсий / КоличествоПотоковВыполнения Тогда
			КоличествоИтераций = КоличествоИтераций + 1;
		КонецЕсли;
		Лог.Информация("->	Количество блоков заданий: " + КоличествоИтераций);
		ТекущийПроход = 1;
		Пока ТекущийПроход <= КоличествоИтераций Цикл
			Лог.Информация("Блок номер " + ТекущийПроход + ", " + ТекущаяДата());
			МассивЗаданий = Новый Массив();
			Для СЧ = 1 По КоличествоПотоковВыполнения Цикл
				...
				Если СтрокаВерсии <> Неопределено Тогда
					...
					ПараметрыПотока.Вставить("СтрокаВерсии", СтрокаВерсии);
					ПараметрыПотока.Вставить("СледующаяВерсия", СледующаяВерсия);
					ПараметрыПотока.Вставить("КаталогРабочейКопии", КаталогРабочейКопии + СЧ);
					ПараметрыПотока.Вставить("ПутьКХранилищу", ПутьКХранилищу);
					ПараметрыПотока.Вставить("ТекущаяВерсия", ТекущаяВерсия);
					
					//TODO: Необходимо перевести в список из файла
					КонтекстВыполненияПотока = Новый Структура("СтрокаСоединения, Пользователь, Пароль", КонтекстВыполнения.СтрокаСоединения + "-" + СЧ);
					ПараметрыПотока.Вставить("Конфигуратор", ПолучитьУправлениеКонфигуратором(КонтекстВыполненияПотока));
					
					Лог.Информация("	->  Запущена рскаладка версии: " + СледующаяВерсия);
					Лог.Информация("	Каталог раскладки: " + КаталогРабочейКопии + СЧ);
					Лог.Информация("	СтрокаСоединения: " + КонтекстВыполненияПотока.СтрокаСоединения);
					
					ПараметрыЗадания = Новый Массив();
					ПараметрыЗадания.Добавить(ПараметрыПотока);
					
					МассивЗаданий.Добавить(ПараметрыПотока);
					
					ФоновыеЗадания.Выполнить(ЭтотОбъект, "ЗапуститьПотокИснхронизации", ПараметрыЗадания);
			...
			Для Каждого ТекущийПоток Из МассивЗаданий Цикл
				ОчиститьКаталогРабочейКопии(КаталогРабочейКопии);
				ПереместитьВКаталогРабочейКопии(КаталогРабочейКопии, ТекущийПоток.КаталогРабочейКопии);
				ВыполнитьКоммитГит(КаталогРабочейКопии,
					ТекущийПоток.СтрокаВерсии.Комментарий,
					ТекущийПоток.СтрокаВерсии.ПредставлениеАвтора,
					ТекущийПоток.СтрокаВерсии.Дата);
				
				ВыполнитьПодпискиПослеКоммита(КаталогРабочейКопии);
				
				УдалитьФайлы(ТекущийПоток.КаталогРабочейКопии);
	...
	Иначе
		Пока СледующаяВерсия <= МаксимальнаяВерсияДляРазбора Цикл
			...
			Если СтрокаВерсии <> Неопределено Тогда
				...
				ПараметрыПотока.Вставить("СтрокаВерсии", СтрокаВерсии);
				ПараметрыПотока.Вставить("СледующаяВерсия", СледующаяВерсия);
				ПараметрыПотока.Вставить("КаталогРабочейКопии", КаталогРабочейКопии);
				ПараметрыПотока.Вставить("ПутьКХранилищу", ПутьКХранилищу);
				ПараметрыПотока.Вставить("ТекущаяВерсия", ТекущаяВерсия);
				ПараметрыПотока.Вставить("Конфигуратор", Неопределено);
				
				ЗапуститьПотокИснхронизации(ПараметрыПотока);
...
#Область Функциональность_обработки_потоков_выполнения

Процедура ЗапуститьПотокИснхронизации(ПараметрыПотока) Экспорт
	
	СтрокаВерсии = ПараметрыПотока.СтрокаВерсии;
	СледующаяВерсия = ПараметрыПотока.СледующаяВерсия;
	ТекущаяВерсия = ПараметрыПотока.ТекущаяВерсия;
	КаталогРабочейКопии = ПараметрыПотока.КаталогРабочейКопии;
	ПутьКХранилищу = ПараметрыПотока.ПутьКХранилищу;
	Конфигуратор = ПараметрыПотока.Конфигуратор;
	...
	Попытка
		...
		ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(КаталогРабочейКопии, ПутьКХранилищу, СледующаяВерсия, Конфигуратор);
...
//	 Конфигуратор - Объект - Рабочий конфигуартор потока при многопоточной работе
//
Процедура ВыгрузитьВерсиюХранилищаКонфигурацииВИсходники(Знач КаталогРабочейКопии,
		Знач ПутьКХранилищу,
		Знач НомерВерсии,
		Знач Конфигуратор = Неопределено) Экспорт
	
	МногопоточныйРежим = Ложь;
	
	Если Конфигуратор = Неопределено Тогда
		Конфигуратор = ПолучитьРабочийКонфигуратор();
	Иначе
		МногопоточныйРежим = Истина;
	КонецЕсли;
	...
	Если МногопоточныйРежим Тогда
		КаталогВыгрузки = КаталогРабочейКопии;
	Иначе
...
//   Объект.УправлениеКонфигуратором - инициализированный класс <УправлениеКонфигуратором>
//
Функция ПолучитьУправлениеКонфигуратором(Знач КонтекстВыполненияПотока = Неопределено) Экспорт
	
	Если КонтекстВыполненияПотока = Неопределено Тогда
		КонтекстВыполненияПотока = КонтекстВыполнения;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(КонтекстВыполненияПотока.СтрокаСоединения) Тогда
		
		Лог.Отладка("Для синхронизации используется база <%1>", КонтекстВыполненияПотока.СтрокаСоединения);
		УправлениеКонфигуратором.УстановитьКонтекст(КонтекстВыполненияПотока.СтрокаСоединения, КонтекстВыполненияПотока.Пользователь, КонтекстВыполненияПотока.Пароль);
